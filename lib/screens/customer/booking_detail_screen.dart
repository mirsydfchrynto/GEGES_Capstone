...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  if (queue.isRefunded == true || queue.status == QueueStatus.cancelled) 
  ...
  Widget _buildStep(String t, String s, bool done, {bool isLast = false, bool isCurrent = false}) {
    return IntrinsicHeight(
      child: Row(
        children: [
          Column(children: [
            Container(width: 14, height: 14, decoration: BoxDecoration(color: done ? kBrownAccent : Colors.white12, shape: BoxShape.circle, border: Border.all(color: done ? kBrownAccent : Colors.white24, width: 2))),
            if (!isLast) Expanded(child: Container(width: 2, color: done ? kBrownAccent : Colors.white12)),
          ]),
          const SizedBox(width: 16),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(t, style: TextStyle(color: done ? Colors.white : Colors.white38, fontWeight: done ? FontWeight.bold : FontWeight.normal, fontSize: 14)),
            Text(s, style: TextStyle(color: done ? kTextGrey : Colors.white10, fontSize: 11)),
            const SizedBox(height: 20),
          ])),
        ],
      ),
  ...
  Widget _buildRefundInfo(Queue queue) {
    if (queue.isRefunded != true) return const SizedBox.shrink();
        ...
        children: [
          Row(
            children: [
              ...
              const Text('INFORMASI REFUND', style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold, fontSize: 14)),
            ],
          ),
          const SizedBox(height: 12),
          Text('Alasan: ${queue.refundReason ?? '-'}', style: const TextStyle(color: Colors.white70, fontSize: 13)),
          ...
          else if (proof != null && proof.isNotEmpty)
            GestureDetector(
