...
- Added GitHub Actions workflow `.github/workflows/integration-e2e.yml` to start the Firestore emulator and run `integration_test/e2e_resume_payment_test.dart` on Android emulator. This workflow now **runs automatically** for pushes and PRs that touch integration-related files (`integration_test/**`, `lib/**`, `backend/functions/**`, `pubspec.yaml`) and remains runnable manually via `workflow_dispatch`. The workflow includes caching, retry logic (3 attempts), log collection, concurrency cancellation, and an increased timeout to reduce flakiness and keep runs efficient.
