...
class FakeQueueService implements QueueServiceContract {
  ...
  @override
  Future<Queue?> resolveQueueForCustomerByIdOrOrder(String idOrOrderId, String customerId) async => byResolve;
  ...
  @override
  Future<void> cancelQueue(String queueId, {String reason = 'Admin/Barberman Cancellation', String? cancelledBy}) async {}
...
void main() {
  testWidgets('PaymentScreen shows uploaded state and disables submit', (WidgetTester tester) async {
    final q = Queue(
      id: 'q-1',
      barbershopId: 'shop-1',
      customerId: 'user1',
      ...
      paymentDeadline: Timestamp.fromDate(DateTime.now().add(const Duration(minutes: 10))),
      paymentProofBase64: 'exists',
    );

    final fake = FakeQueueService(byId: q, byResolve: q);

    await tester.pumpWidget(MaterialApp(home: PaymentScreen(orderId: 'order-1', totalPrice: 50000, queueService: fake, testUserId: 'user1')));
