...
void main() {
  group('QueueService helpers', () {
    test('isBookingLeadTimeSufficient returns false for booking < 30 minutes', () {
      final now = DateTime.now();
      final booking = now.add(const Duration(minutes: 10));
      final ok = QueueService.isBookingLeadTimeSufficient(booking, minMinutes: 30);
      expect(ok, false);
    });

    test('isBookingLeadTimeSufficient returns true for booking >= 30 minutes', () {
      final now = DateTime.now();
