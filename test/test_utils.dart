...
Future<void> pumpAndSettleWithTimeout(WidgetTester tester, {Duration timeout = const Duration(seconds: 5)}) async {
  ...
  throw TimeoutException('pumpAndSettleWithTimeout exceeded ${timeout.inSeconds}s');
}

/// Create a lightweight temp file with given bytes; returns the [File] instance.
Future<File> createTempFile(List<int> bytes, {String prefix = 'tmp', String suffix = '.dat'}) async {
  final dir = Directory.systemTemp.createTempSync(prefix);
  final f = File('${dir.path}/file${DateTime.now().millisecondsSinceEpoch}$suffix');
