...
void main() {
  testWidgets('TC-LOGIN-GOOGLE-01: Google sign-in success navigates to Home via homeBuilder', (WidgetTester tester) async {
    final fake = FakeAuthServiceSpy(googleResponseOverride: {'success': true, 'role': 'customer'});

    await tester.pumpWidget(
      MaterialApp(
        home: LoginScreen(
          authService: fake,
          homeBuilder: (_) => const Scaffold(body: Center(child: Text('HomeScreenFake'))),
        ),
      ),
    );
  ...
  testWidgets('TC-LOGIN-GOOGLE-02: Google sign-in failure shows Retry action for credential/recaptcha errors', (WidgetTester tester) async {
    final fake = FakeAuthServiceSpy(googleResponseOverride: {'success': false, 'message': 'recaptcha token kosong'});

    await tester.pumpWidget(
      MaterialApp(
        home: LoginScreen(authService: fake),
      ),
    );
    ...
    expect(find.textContaining('recaptcha', findRichText: true), findsWidgets);
  ...
  testWidgets('TC-LOGIN-FORGOT-01: Forgot password dialog and send reset email', (WidgetTester tester) async {
    ...
    final emailField = find.descendant(of: find.byType(AlertDialog), matching: find.byType(TextField));
