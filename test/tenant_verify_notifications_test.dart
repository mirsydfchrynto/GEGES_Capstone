...
class FakeEmailOutbox {
  ...
  Future<void> queueEmail({required String to, required String subject, required String body, Map<String, dynamic>? metadata}) async {
    queued = true;
    lastArgs = {'to': to, 'subject': subject, 'body': body, 'metadata': metadata};
  }
}

void main() {
  test('verifyTenant creates notification and queues email when owner info present (approve)', () async {
    final fs = FakeFirebaseFirestore();
    final fakeOutbox = FakeEmailOutbox();
    final service = TenantService(firestore: fs, emailOutboxService: fakeOutbox);
    ...
    await service.verifyTenant(tenantId: 't-approve', approve: true, verifiedBy: 'admin1');

    final tdoc = await tenantRef.get();
    expect(tdoc.data()!['status'], 'active');

    // notification added
    final notifs = await fs.collection('notifications').where('user_id', isEqualTo: 'owner123').get();
  ...
  test('verifyTenant creates notification and queues email when owner info present (reject)', () async {
    ...
    await service.verifyTenant(tenantId: 't-reject', approve: false, verifiedBy: 'admin2', reason: 'invalid docs');
